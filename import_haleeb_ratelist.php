...
$columns = [];
$colRes = $conn->query("SELECT id, column_key, column_label, is_hidden, display_order FROM haleeb_rate_list_columns WHERE is_deleted=0 ORDER BY is_hidden ASC, display_order ASC, id ASC");
...
if(count($columns) === 0){
fclose($handle);
header("location:haleeb_ratelist.php?import=error&reason=no_columns");
exit();
}
...
header("location:haleeb_ratelist.php?import=success&ins=$inserted&skip=$skipped");
exit();
?>
