        ...
        if(!auth_can_direct_modify()){
            ...
            try {
                ...
                if($remainingAfter <= 0.0001){
                    $paidType = 'paid';
                    $paidStmt = $conn->prepare("UPDATE bilty SET freight_payment_type=? WHERE id=?");
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .info-val.yellow { color: var(--accent); }
  ...
  .field input, .field select {
    ...
    padding: 10px 12px; font-family: var(--font); font-size: 13px; transition: border-color 0.15s; appearance: none;
  }
