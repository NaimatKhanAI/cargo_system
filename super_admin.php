...
function table_row_count_local($conn, $table){
...
function run_query_list_local($conn, $queries, &$queryError){
...
function current_db_name_local($conn){
...
if(isset($_POST['run_db_action'])){
    ...
    if(!auth_is_super_admin()){
        $err = 'Only super admin can run database actions.';
    } elseif($actionKey === '' || !isset($dbActionConfig[$actionKey])){
        ...
        if(strtoupper($confirmText) !== strtoupper($expected)){
            ...
            if($mode === 'rebuild_database'){
                ...
                $dropError = '';
                if(run_query_list_local($conn, $dropAllQueries, $dropError)){
                ...
                } elseif(run_query_list_local($conn, $queries, $queryError)){
...
$currentDbName = current_db_name_local($conn);

$dbTableCounts = [
    'users' => table_row_count_local($conn, 'users'),
    'bilty' => table_row_count_local($conn, 'bilty'),
    'haleeb_bilty' => table_row_count_local($conn, 'haleeb_bilty'),
    'account_entries' => table_row_count_local($conn, 'account_entries'),
    'image_processed_rates' => table_row_count_local($conn, 'image_processed_rates'),
    'haleeb_image_processed_rates' => table_row_count_local($conn, 'haleeb_image_processed_rates'),
    'change_requests' => table_row_count_local($conn, 'change_requests'),
    'activity_notifications' => table_row_count_local($conn, 'activity_notifications'),
];
...
<div class="topbar">
  ...
  <div class="nav-links">
    <a class="nav-btn" href="#db-settings">Settings</a>
...
<div class="main">
  ...
  <div class="section" id="db-settings">
    <div class="section-head">
      <span class="section-title">Database Settings</span>
